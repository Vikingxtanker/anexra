...
export default async function DPharmPage() {
  await connection();

  const program = pharmacyPrograms["D.Pharm"];
    ...
    <section className="min-h-screen bg-gradient-to-br from-[#f4efee] via-white to-[#f4efee] px-6 py-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {years.map((year) => {
            const subjectCount =
              program.years[year as keyof typeof program.years]
                .subjects.length;

            return (
              <Link
                key={year}
                href={`/student/dashboard/dpharm/${year
                    .toLowerCase()
                    .replace(/\s+/g, "-")}`}
                >
                <div
